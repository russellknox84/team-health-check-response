...
const getQuestions = () => axios.get("/api/questions")
const getProjects = () => axios.get("/api/projects")
const getSurveys = () => axios.get("/api/surveys")

axios.all([getQuestions(), getProjects(), getSurveys()])
    .then(axios.spread(function (question, project, survey) {
        ...
        const projectIds = project.data.map(ids => ids.projectName)
        const projects = project.data.reduce((state, value) => {
